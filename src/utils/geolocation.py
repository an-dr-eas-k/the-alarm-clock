...
class WMO_Code:
    ...
    def to_character(self):
        weather_icon_element = weather_icons_tree.find(
            f".//string[@name='wi_wmo4680_{self.code}']"
        )
...
class GeoLocation:
    ...
    def last_sun_event(self, dt: datetime.datetime = None) -> SunEvent:
        ...
        localSun = sun(
            ...
            date=dt.date(),
            tzinfo=self.location_info.tzinfo,
        )
        return (
            SunEvent.sunrise
            if dt > localSun[SunEvent.sunrise.value]
            and dt < localSun[SunEvent.sunset.value]
