...
class AnalogClockGenerator:
		...
		def get_current_clock(self, now: datetime = datetime.now(), clock_radius: int = 31) -> Image.Image:
		...
		def get_clock(self, hour: int, minute: int, second: int, clock_radius: int = 31) -> Image.Image:
				
				canvas = Image.new("RGBA", (clock_radius*2+1, clock_radius*2+1), color=self.background_color)
